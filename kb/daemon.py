...
class KBHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        strict_mode     = bool(body.get("strict_mode",     CONFIG["strict_mode"]))
        strict_cds_mode = bool(body.get("strict_cds_mode", CONFIG["strict_cds_mode"]))
        ...
        result = RETRIEVER.search(
            ...
            strict_cds_mode=strict_cds_mode,
...
def main() -> None:
    ...
    CONFIG = {
        ...
        "snippet_chars": 15000,
        "source_mode": "who",
        "threshold": 0.0,        # no hard floor; caller can set threshold explicitly
        "search_mode": "rrf",    # hybrid semantic + BM25
        "strict_mode": False,    # F10: if True, zero out hits when quality_flags triggered
        "strict_cds_mode": False, # precision policy: action types only, no min-keep, hard title block
        "safe_top1_guardrail": False, # staged rollout: safety top1 swap only when explicitly enabled
    }
    VEC_INDEX = "/var/www/kbToolUseLora/kb/who_knowledge_vec.mv2"
    RETRIEVER = KBRetriever(who_index=VEC_INDEX)
